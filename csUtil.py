...
class DataUploader:
    ...
    def __init__(self, api, interval=30):
        ...
        self.uploadStop = threading.Event()
        self.uploadThread = threading.Thread(target=self.run)
        self.uploadThread.setDaemon(True)
    ...
    def run(self):
        while not self.uploadStop.is_set() or self.dataQueue.qsize() > 0:
            while self.dataQueue.qsize() > 0:
                try:
                    self.upload()
                except SSLError:
                    print "SSLError."
            self.uploadStop.wait(self.interval)
...
def getSensorList(api, sensorName, deviceType=None, description=None, userName=None, deviceUUID=None):
    ...
    if deviceUUID:
        correctSensors = filter(lambda x: "device" in x and x['device']['uuid'] == deviceUUID, correctSensors)
    if description:
...
def getSensorId(api, sensorName, deviceType=None, description=None, userName=None, deviceUUID=None):
    correctSensors = getSensorList(api, sensorName, deviceType, description, userName, deviceUUID)
...
def getSensorsData_inner(api, sensorIds, fromDate=None, tillDate=None, refreshInterval=None):
    ...
    start_date = None
    if fromDate is not None:
        start_date = time.mktime(fromDate.timetuple())
    ...
    while (True):
        if start_date is not None:
            ...
            for x in response['data']: 
                start_date = max(start_date, x['date'])
                yield x
            #see whether all data is got
            nr = len(response['data'])
            if (nr < par['per_page']):
